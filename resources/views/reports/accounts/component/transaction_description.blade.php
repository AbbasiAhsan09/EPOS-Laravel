...
@dump($data->reference_type ?? "") --}}
    ...
    @if ($data && isset($data->sale_return) && $data->reference_type === 'sales_return')
        <p>Sale Return# {{ $data->sale_return->doc_no }} Dated
            {{ date('d/m/Y', strtotime($data->sale_return->return_date)) }} </p>
        @if ($data->sale_return && count($data->sale_return->order_details))
            @foreach ($data->sale_return->order_details as $detail)
                <p>{{ $detail->item_details->fullProductName ?? '' }} - W {{ $detail->returned_qty }}
                    {{ '@' . ConfigHelper::getStoreConfig()['symbol'] . $detail->returned_rate }}</p>
            @endforeach
        @endif
    ...
    @if ($data && isset($data->purchase_return) && $data->reference_type === 'purchase_return')
        <p>Purchase Return# {{ $data->purchase_return->doc_no }} Dated
            {{ date('d/m/Y', strtotime($data->purchase_return->return_date)) }} </p>
    ...
    @if ($data && isset($data->purchase) && $data->reference_type === 'purchase_invoice')
        <p>Purchase#{{ $data->purchase->doc_num }} -
            {{ $data->purchase->condition ? 'On condition ' . $data->purchase->condition : '' }} Dated
            {{ date('d/m/Y', strtotime($data->purchase->bill_date ? $data->purchase->bill_date : $data->purchase->created_at)) }}
        ...
        @if ($data->purchase && count($data->purchase->details))
            @foreach ($data->purchase->details as $detail)
                <p>{{ $detail->items->fullProductName ?? '' }} - W {{ $detail->qty }}
                    {{ '@' . ConfigHelper::getStoreConfig()['symbol'] . $detail->rate }}</p>
            @endforeach
        @endif
...
{{-- Vouchers --}}
    @if ($data && isset($data->voucher) && $data->reference_type === 'voucher')
        <p>{{ $data->voucher->doc_no }} - Mode : {{ Str::ucfirst($data->voucher->mode) }}</p>
        @if ($data->voucher && count($data->voucher->entries) > 1)
            @foreach ($data->voucher->entries as $key => $detail)
                <p> {{ $key+1 }}).  {{ $detail->reference ? $detail->reference . " - " : "" }} {{ $detail->description ? $detail->description . " - " : ""  }} {{ number_format($detail->amount,2) }}</p>
            @endforeach
        @endif
    ...
    @if (
        $data &&
            ...
            (!(isset($data->sale) && $data->reference_type === 'sales_order') &&
                ...
                !(isset($data->voucher) && $data->reference_type === 'voucher') &&
