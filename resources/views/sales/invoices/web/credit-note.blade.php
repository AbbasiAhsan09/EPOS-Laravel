...
<div class="receipt-content">
    ...
    <div class="container bootstrap snippets bootdey ">
		...
		<div class="row">
            ...
            <strong>Sale Returned Invoice </strong>
					</div>
					...
					<div class="payment-details">
						<div class="row">
							<div class="col-sm-6">
								...
								@if (isset($order->party->business_name) && $order->party->business_name)
                <strong style="font-size: 20px">
									Business Name : {{isset($order->party ) ? $order->party->business_name : '' }}
								</strong>
                @endif
								<p>
                  @if (isset($order->party ))
                  <strong style="font-size: 20px">Party Name: {{$order->party->party_name}}</strong> <br>
                  @if ($order->party->phone)
                    {{$order->party->phone}}                      
                  @endif
                  @if ($order->party->location)
                      {{$order->party->location}}
                  @endif
                  @if ($order->party->email)
                      {{$order->party->email}}
                  @endif
                  @else
                ...
                <strong>Return No. {{$order->doc_no ?? ""}}</strong>
                  @if ($order->invoice_no)                      
                  <br>
									<strong>Bill No. {{$order->invoice_no ?? ""}}</strong>
              ...
              <p class="extra-notes">
                ...
                @endif
                 --}}
							<div class="field">
								Subtotal <span>{{ConfigHelper::getStoreConfig()["symbol"].number_format(round($order->order_details->sum("returned_total")))}}</span>
							</div>
                            @if ( $order->other_charges > 0)
							<div class="field">
								Bardana Charges <span>{{ConfigHelper::getStoreConfig()["symbol"].number_format(round($order->other_charges))}}</span>
							</div>
                            @endif
                            @if ($order->discount > 0)
							<div class="field">
							...
							<div class="field grand-total">
								Returned Total. <span>{{ConfigHelper::getStoreConfig()["symbol"].number_format(round($order->net_total))}}</span>
							</div>
