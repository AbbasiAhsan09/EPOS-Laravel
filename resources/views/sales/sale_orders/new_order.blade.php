                ...
                <div class="col-lg-12">
                    ...
                    <div class="mid-section">
                        <div class="row align-items-center">
                            <div class="col-lg-2">
                                <h1 class="page-title">{{ $isEditMode ? 'Edit' : 'Create' }} Order
                                    {{ $isEditMode ? ' : ' . $order->tran_no ?? '' : '' }}</h1>
                                ...
                                <div class="order-meta">
                                    <div class="row">
                                        <div class="col-lg-2">
                                            ...
                                            <div
                                                class="input-group input-group-outline w-100 d-flex align-items-center justify-content-center">
                                                <input type="text" id="search_value" class="form-control"
                                                    value="{{ $isEditMode ? $order->tran_no : '' }}">
                                                <button type="button" class="btn btn-small btn-primary" style="margin: 0"
                                                    id="search_order_button"><i class="fa fa-search"></i></button>
                                            </div>
                                        </div>
                                        @if (
                                            ($config->bill_date_changeable && Auth::user()->userroles->role_name == 'Admin') ||
                                                Auth::user()->userroles->role_name == 'SuperAdmin')
                                            <div class="col-lg-2">
                                                <div class="bill-date-wrapper">
                                                    ...
                                                    <div class="input-group input-group-outline">
                                                        <input type="date" name="bill_date" class="form-control"
                                                            id=""
                                                            value="{{ $isEditMode ? $order->bill_date : date('Y-m-d', time()) }}">
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                                        ...
                                        <div class="col-lg-2">
                                            ...
                                            <div class="select_party_wrapper">
                                                <h4
                                                    class="order_section_sub_title d-flex align-items-center justify-content-between">
                                                    <span>Select Party </span> <i style="cursor: pointer;"
                                                        class="fa fa-plus" data-bs-toggle="modal"
                                                        data-bs-target="#addPartyModal"></i>
                                                </h4>
                                        ...
                                        <div class="col-lg-2">
                                            {{-- Total  --}}
                                            <div class="order_total_wrapper my-3 px-2"
                                                style="border: solid 2px; background : rgb(222, 26, 26)">
                                                <div class="order_total">
                                                    <h3 class="page-title text-primary" style="color: white !important">
                                                        Total: {{ ConfigHelper::getStoreConfig()['symbol'] }} <span
                                                            class="g_total ">{{ $isEditMode ? $order->net_total : 0 }}</span>
                                                    </h3>
                        ...
                        <div class="new_order_item_selection_wrapper">
                            ...
                            <div class="new_order_item_selection">
                                ...
                                <input type="hidden" value="{{ $config->show_tp_in_order_form }}"
                                    id="show_tp_in_order_form">
                                        ...
                                        @if ($isEditMode && count($order->order_details))
                                            @foreach ($order->order_details as $item)
                                             ...
                                             @endif --}}
                                                    ...
                                                    <td><input name="rate[]" type="number" step="0.01"
                                                            placeholder="Rate" min="1" class="form-control rate"
                                                            value="{{ $item->rate }}"></td>
                                                    <td><input name="qty[]" type="number" step="0.01"
                                                            data-item-id="{{ $item->item_details->id }}"
                                                            placeholder="Qty" min="1"
                                                            class="form-control pr_qty {{ $isEditMode ? 'edit_qty' : '' }}"
                                                            value="{{ $item->qty }}"></td>
                                                    <td><input name="tax[]" type="number" step="0.01"
                                                            placeholder="Tax" min="0" class="form-control tax"
                                                            value="{{ $item->tax }}"></td>
                                            ...
                                            <div class="row">
                                                <div class="col">
                                                    <input type="text" name="discount" id="discount"
                                                        ...
                                                        value="{{ $isEditMode ? ($order->discount_type === 'PERCENT' ? '%' . $order->discount : $order->discount) : '%' }}"
                                                        onkeypress="validationForSubmit()">
                                                </div>
                                                <div class="col" id="discountSection"
                                                    style="display: {{ $isEditMode && $order->discount_type === 'PERCENT' ? 'block' : 'none' }}">
                                                    <input type="number" id="discountValue" disabled readonly
                                                        class="form-control" required
                                                        value="{{ $isEditMode && $order->discount_type === 'PERCENT' ? number_format(($order->gross_total / 100) * $order->discount, 2) : 0 }}"
                                                        onkeypress="validationForSubmit()">
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-lg-1">
                                        ...
                                        <div class="input-group input-group-outline">
                                            <input type="number" name="other_charges" id="otherCharges"
                                                class="form-control" required
                                                value="{{ $isEditMode ? $order->other_charges : 0 }}" min="0"
                                                onkeypress="validationForSubmit()">
                                        </div>
                                    </div>
                                    {{-- @if (!$isEditMode)
                                    ...
                                    <div class="col-lg-1">
                                        ...
                                        <div class="input-group input-group-outline">
                                            <input type="number" name="recieved" id="received-amount"
                                                class="form-control" required
                                                value="{{ $isEditMode ? $order->recieved : 0 }}" min="0"
                                                onkeypress="validationForSubmit()">
                                        </div>
                                    </div>
                                    <div class="col-lg-2">
                                        ...
                                        <div class="input-group input-group-outline">
                                            <input type="number" class="form-control" disabled readonly
                                                value="{{ $isEditMode ? round($order->recieved - $order->net_total) : 0 }}"
                                                id="returning-amount">
                                        </div>
                                    </div>
                                    <div class="col-lg-2">
                                            ...
                                            <div class="payment_methods">
                                                <label for="cash" class="order-type-item">
                                                    <input type="radio" name="payment_method" id="cash"
                                                        value="cash" class="form-check-input order_type_val"
                                                        {{ $isEditMode ? ($order->payment_method == 'cash' ? 'checked' : '') : 'checked' }}>
                                                    Cash
                                                </label>
                                                <label for="card" class="order-type-item">
                                                    <input type="radio" name="payment_method" id="card"
                                                        value="card" class="form-check-input order_type_val"
                                                        {{ $isEditMode ? ($order->payment_method == 'card' ? 'checked' : '') : '' }}>
                                                    Card
                                                </label>

                                                <label for="Bank" class="order-type-item other-methods">
                                                    <input type="radio" name="payment_method" id="Bank"
                                                        value="bank" class="form-check-input order_type_val "
                                                        {{ $isEditMode ? ($order->payment_method == 'bank' ? 'checked' : '') : '' }}>
                                                    Bank
                                                </label>

                                                <label for="Cheque" class="order-type-item other-methods">
                                                    <input type="radio" name="payment_method" id="Cheque"
                                                        value="cheque" class="form-check-input order_type_val "
                                                        {{ $isEditMode ? ($order->payment_method == 'cheque' ? 'checked' : '') : '' }}>
                                                    Cheque
                                                </label>
                                    ...
                                    <div class="col-lg-3">
                                        ...
                                        <div class="input-group input-group-outline">
                                            <input type="text" name="broker" id="broker" class="form-control"
                                                value="{{ $isEditMode ? $order->broker : '' }}" min="0"
                                                onkeypress="validationForSubmit()">
                                        </div>
                                    </div>
                                    <div class="col-lg-3">
                                        ...
                                        <div class="input-group input-group-outline">
                                            <input type="text" name="condition" id="condition" class="form-control"
                                                value="{{ $isEditMode ? $order->condition : '' }}" min="0"
                                                onkeypress="validationForSubmit()">
                                        </div>
                                    </div>
                                    <div class="col-lg-3">
                                        ...
                                        <div class="input-group input-group-outline">
                                            <input type="text" name="gp_no" id="gp_no" class="form-control"
                                                value="{{ $isEditMode ? $order->gp_no : '' }}" min="0"
                                                onkeypress="validationForSubmit()">
                                        </div>
                                    </div>
                                    <div class="col-lg-2">
                                        ...
                                        <div class="input-group input-group-outline">
                                            <input type="text" name="truck_no" id="truck_no" class="form-control"
                                                value="{{ $isEditMode ? $order->truck_no : '' }}" min="0"
                                                onkeypress="validationForSubmit()">
                                        </div>
                                    </div>
                                    <div class="col-lg">
                                        ...
                                        <label for="print_invoice" class="order-type-item">
                                            <input type="checkbox" name="print_invoice" value="1"
                                                id="print_invoice" class="form-check-input order_type_val">
                                            <i class="fa fa-print"></i>
                                        </label>
                                    </div>
                                    <div class="col-lg-12">
                                        <button class="btn btn-block btn-primary btn-lg" id="saveOrderBtn"
                                            style="width: 100%" disabled>Proceed</button>
        ...
        @if (session('openNewWindow'))
            <script>
                $(document).ready(function() {
                    ...
                    window.open("{{ url('/invoice/' . session('openNewWindow') . '') }}", "popupWindow",
                        "width=300,height=600,scrollbars=yes,left=" + ($(window).width() - 400) + ",top=50");
                })
            </script>
        @endif
    @section('scripts')
        ...
        <script>
            ...
            document.addEventListener("keydown", function(event) {
                if (event.altKey && event.key === "a") {
                    ...
                    if (form.checkValidity() && !inValid && !isNaN(grossTotalNumber) && grossTotalNumber > 0 &&
                        submitted === 0) {
